#!/usr/bin/env node
// Example art fetcher (albums): pulls cover art from the iTunes Search API and
// writes art-cache.js. Usage: node fetch-art.js
//
// Reads window.ITEMS from items.js, queries iTunes for each, and produces a JS
// file that maps item IDs to artwork URLs. The generated file is loaded by the
// app so it works offline from the local file system.
//
// Art sourcing is inherently media-specific — this iTunes lookup is the album
// example. For other media types, replace fetchArt() with the right source
// (e.g. a BoardGameGeek collection export, or IMDb poster URLs pasted by hand).

const fs = require("fs");
const path = require("path");

// --- Load items from items.js (which sets window.ITEMS) ---
const itemsSrc = fs.readFileSync(path.join(__dirname, "items.js"), "utf-8");
// Execute in a mini-context with a window shim to extract the array.
const fn = new Function("window", itemsSrc + "\nreturn window.ITEMS;");
const ALBUMS = fn({});
...
async function main() {
  ...
  const cachePath = path.join(__dirname, "art-cache.js");
  let existing = {};
  ...
  console.log(
    `\nDone. ${fetched} fetched, ${skipped} cached, ${failed} not found. ` +
      `${Object.keys(cache).length} total entries written to art-cache.js`
  );
}
